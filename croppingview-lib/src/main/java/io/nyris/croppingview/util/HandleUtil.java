...
package io.nyris.croppingview.util;
...
public class HandleUtil {
    ...
    public static void normalizeRectF(RectF rectF, int srcWidth, int srcHeight, int destWidth, int destHeight) {
        Matrix matrixTransform = getTransformationMatrix(
                srcWidth,
                srcHeight,
                destWidth,
                destHeight);
        ...
        if (rectF.top < 0)
            rectF.top = 0;
        if (rectF.bottom > destHeight)
            rectF.bottom = destHeight;
        if (rectF.right > destWidth)
            rectF.right = destWidth;
    }
}
